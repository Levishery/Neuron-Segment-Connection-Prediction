...
# from models import GroupFreeVectorDetector, get_loss, get_loss_vector, GroupFreeVectorDetector_ptv3
from models import get_loss, get_loss_vector, GroupFreeVectorDetector_ptv3
...
def parse_option():
    ...
    parser.add_argument('--candidate_record_path', default='candidate_record', type=str, help='path to record candidate')
    parser.add_argument('--candidate_scan_area', default='cone', type=str, help='difine the scan area as cone / cylinder/ sphere')
    parser.add_argument('--test_target', default='None', type=str, help='difine the plot target as dist / radius')
...
def get_loader(args):
    ...
    if args.dataset == 'vector':
        ...
        DATASET_CONFIG = VectorDatasetConfig(test_target=args.test_target)
    ...
    print(TRAIN_DATASET.split_set, VAL_DATASET.split_set, TEST_DATASET.split_set)
    ...
    test_loader = torch.utils.data.DataLoader(TEST_DATASET,
                                              ...
                                              drop_last=False)
    print(f"train_loader_len: {len(train_loader)}, val_loader_len: {len(val_loader)}, test_loader_len: {len(test_loader)}")
...
def get_model(args, DATASET_CONFIG):
    model = GroupFreeVectorDetector_ptv3(num_yaw_bin=DATASET_CONFIG.num_yaw_bin,
...
def test_one_epoch(test_loader, DATASET_CONFIG, model, config, summary_writer, epoch, get_terminal_thresh=False, save_visualization=False):
