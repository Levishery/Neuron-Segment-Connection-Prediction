...
def encode(locs, num_dims, num_bits):
    ...
    # torch.view(torch.unit8) needs torch>=1.12.0, so change it to numpy
    locs = locs.long()
    gpu = locs.device
    locs_uint8 = torch.tensor(np.asarray(locs.cpu()).view(np.uint8)).to(gpu)
    ...
    hh_uint64 = torch.tensor(np.asarray(hh_uint8.cpu()).view(np.int64)).to(gpu).squeeze()
